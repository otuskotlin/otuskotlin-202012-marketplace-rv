...
import ru.otus.otuskotlin.marketplace.rv.business.logic.backend.operations.stubs.ArtCreateStub

object ArtCreate: IOperation<MpBeContext> by pipeline( {
    execute(InitializePipeline)
    execute(QuerySetWorkMode)
    execute(ArtCreateStub)
    ...
    operation {
        ...
        execute {
            try {
                ...
                errors.add(
                    MpError(
                        code = "couldn't create art(repo error)",
                        message = t.message?:"")
                )
            }
        }

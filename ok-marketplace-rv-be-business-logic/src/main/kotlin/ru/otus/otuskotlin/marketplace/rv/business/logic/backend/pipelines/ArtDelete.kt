...
import ru.otus.otuskotlin.marketplace.rv.business.logic.backend.operations.stubs.ArtDeleteStub

object ArtDelete: IOperation<MpBeContext> by pipeline( {
    execute(InitializePipeline)
    execute(QuerySetWorkMode)
    execute(ArtDeleteStub)
    ...
    operation {
        ...
        execute {
            try {
                ...
                errors.add(
                    MpError(
                        code = "couldn't delete art(repo error)",
                        message = t.message?:"")
                )
            }
        }
